...
var _createType = _interopRequireDefault(require("./createType"));
...
describe('Vector', () => {
  let vector;
  beforeEach(() => {
    vector = new _Vector.default(_Text.default, ['1', '23', '345', '4567', new _Text.default('56789')]);
  ...
  it('allows contruction via JSON', () => {
    expect(new _Vector.default(_Text.default, ['6', '7']).toJSON()).toEqual(['6', '7']);
  });
  it('exposes the type', () => {
    expect(vector.Type).toEqual('Text');
  });
  it.skip('decodes a complex type via construction', () => {
    const test = (0, _createType.default)('Vec<(PropIndex, Proposal, AccountId)>', new Uint8Array([4, 10, 0, 0, 0, 0, 3, 80, 123, 10, 9, 34, 48, 120, 52, 50, 34, 58, 32, 34, 48, 120, 52, 51, 34, 10, 125, 10, 209, 114, 167, 76, 218, 76, 134, 89, 18, 195, 43, 160, 168, 10, 87, 174, 105, 171, 174, 65, 14, 92, 203, 89, 222, 232, 78, 47, 68, 50, 219, 79]));
    const first = test[0];
    expect(first[0].toNumber()).toEqual(10);
    expect(first[1].callIndex).toEqual(new Uint8Array([0, 3]));
    expect(first[2].toString()).toEqual('5GoKvZWG5ZPYL1WUovuHW3zJBWBP5eT8CbqjdRY4Q6iMaDtZ');
  });
  ...
  describe('utils', () => {
    const vec = new _Vector.default(_Text.default, ['123', '456']);
    ...
    it('finds the index of an value', () => {
      ...
      const vec = new _Vector.default(_AccountId.default, ['5HGjWAeFDfFCWPsjFQdVV2Msvz2XtMktvgocEZcCj68kUMaw', '5FHneW46xGXgs5mUiveU4sbTyGBzmstUspZC92UhjJM694ty', '5GrwvaEF5zXb26Fz9rcQpDWS57CtERHpNehXCPcNoHGKutQY']);
