...
describe('Set', () => {
  it('constructs via an Array<string>', () => {
    const set = new _Set.default(SET_ROLES, ['full', 'authority']);
    expect(set.isEmpty).toEqual(false);
    expect(set.toString()).toEqual('[full, authority]');
  });
  it('throws with invalid values', () => {
    expect(() => new _Set.default(SET_ROLES, ['full', 'authority', 'invalid'])).toThrow(/Invalid key 'invalid'/);
  });
  it('throws with add on invalid', () => {
    expect(() => new _Set.default(SET_ROLES, []).add('invalid')).toThrow(/Invalid key 'invalid'/);
  });
  it('allows construction via number', () => {
    expect(new _Set.default(SET_WITHDRAW, 15).eq(['TransactionPayment', 'Transfer', 'Reserve', 'Fee'])).toBe(true);
  });
  it('does not allow invalid number', () => {
    expect(() => new _Set.default(SET_WITHDRAW, 31)).toThrow(/Mismatch decoding '31', computed as '15'/);
  });
  it('hash a valid encoding', () => {
    const set = new _Set.default(SET_FIELDS, ['header', 'body', 'justification']);
  ...
  describe('utils', () => {
    ...
    it('compares against other sets', () => {
      expect(set.eq(new _Set.default(SET_ROLES, ['authority', 'full']))).toBe(true);
    });
