...
class AddressBook extends Bond {
  ...
  find(identifier) {
    ...
    if (
      (identifier instanceof Uint8Array && identifier.length == 32) ||
      identifier instanceof AccountId
    ) {
      identifier = ss58Encode(identifier);
    }
    return this._byAddress[identifier]
      ? this._byAddress[identifier]
      : this._byName[identifier];
  }
  ...
  _sync() {
    ...
    this._byName = byName;
    localStorage.addressBook = JSON.stringify(
      this._accounts.map(k => ({ address: k.address, name: k.name }))
    );
    this.trigger({
      accounts: this._accounts,
      byAddress: this._byAddress,
      byName: this._byName
    });
  }
}
