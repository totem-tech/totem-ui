...
const TimeSince = props => {
    ...
    autoUpdate && useEffect(() => {
        ...
        const update = delay => setTimeout(() => {
            if (!mounted) return
            const [formatted, updateFrequency] = formatSince(time)
            setFormatted(formatted)
            updateFrequency && update(updateFrequency)
        }, delay)
        update(delayMS)
...
export const formatSince = time => {
    ...
    if (days < 30) return [fmt(days, 'day'), null] //1000 * 60 * 60 * 24
    const months = days / 30
    if (months < 12) return [fmt(months, 'month'), null] // 1000 * 60 * 60 * 24
    const years = months / 12
    return [fmt(years, 'year'), null] //1000 * 60 * 60 * 24 * 12
}
const fmt = (num, unit) => {
