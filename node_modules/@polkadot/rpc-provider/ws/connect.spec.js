...
describe('onConnect', () => {
  ...
  it('Does connect when autoConnect is true', () => {
    const provider = new _.default(_mockWs.TEST_WS_URL, true); // @ts-ignore

    expect(provider.websocket).not.toBeNull();
  });
  it('Creates a new WebSocket instance by calling the connect() method', () => {
    const provider = new _.default(_mockWs.TEST_WS_URL, false); // @ts-ignore

    expect(provider.websocket).toBeNull();
    provider.connect(); // @ts-ignore

    expect(provider.websocket).not.toBeNull(); // @ts-ignore

    expect(provider.websocket instanceof WebSocket).toBe(true);
  });
  it('Creates the on handlers', () => {
    const provider = new _.default(_mockWs.TEST_WS_URL); // @ts-ignore
