...
export const getTaskList = (targetIdentity) => {
    ...
    const partnersAdded = Array
        .from(partners.getAll())
        .map(([_, partner]) =>
            partnerIds.includes(partner.userId)
                && partner
        )
        .filter(Boolean)
    ...
    historyArr2d
        .forEach(([_, { action, data = [], identity, status }]) => {
            const [recipient] = data
            
            if (targetIdentity !== identity || status !== 'success') {
    ...
    const tasks = [
        {
            ...
            question: (
                <span>
                    Request identity from a friend {partnerRequested && checkMark}
                </span>
            ),
        },
        {
            ...
            question: (
                <span>
                    Add identity shared by a friend {taskStatus.partnersAdded && checkMark}
                </span>
            ),
        },
        {
            answer: (
                <div>
                    ...
                    <ul>
                        ...
                        <li>Select your friend from the recipient DropDown list</li>
                        <li>
                            Enter any amount you wish to send. Amounts transferred will not affect the amount claimable.
                        </li>
                        <li>Submit</li>
                    </ul>
                    Use the transfer module to send funds to one of your friends. 
                </div>
            ),
            completed: taskStatus.transferred,
            question: (
                <span>
                    Transfer any amount to one of your friend's identity {taskStatus.transferred && checkMark}
                </span>
            ),
        },
        {
            answer: (
                <div>
                    ...
                    <ul>
                        ...
                        <li>Enter any name and description for the activity.</li>
                        <li>Submit and wait until Activity is successfully created.</li>
        ...
        },
        {
            ...
            question: (
                <span>
                    Create a timekeeping record {false && checkMark}
                </span>
            ),
        },
        {
...
const getInitialState = rxSetState => {
    const inputs = () => {
        const getSelected = () => identities
            .getSelected()
            .address
        const rewardsIdentity = storage.settings.module('messaging').user.address
        const rewardsIdentityAvailable = !!identities.get(rewardsIdentity)
        const switchIdenity = rewardsIdentityAvailable && rewardsIdentity !== getSelected()
        ...
        return [
            ...
            },
            {
                ...
                label: (
                    <div>
                        In order claim KAPEX you must complete the following tasks: <Icon {...{
                            className: 'no-margin',
                            name: 'refresh',
                            onClick: () => reload(),
                            style: {
            ...
            },
            {
                ...
                minLength: 50,
                name: 'feedback',
                placeholder: 'Please enter your unique feedback including any bug report here (between 50 and 5000 characters)',
                required: true,
                type: 'textarea',
            },
        ]
