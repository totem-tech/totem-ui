...
const InboxList = ({ inboxKey }) => {
    inboxKey = useRxSubjectOrValue(inboxKey || rxOpenInboxKey)
    ...
    const [query = '', setQuery, rxQuery] = useRxSubject(null, null, '')
    const {
        items = [],
        // query = '',
        showAll = false,
                ...
                <div className='list'>
                    {items.map(item => (
                        <InboxListItem {...{
                            ...
                            key: JSON.stringify(item),//item.inboxKey,// + inboxKey === item.inboxKey ? 'active' : 'inactive',
...
const Highlight = React.memo(({
    ...
    query = useRxSubjectOrValue(query)
    const index = `${text || ''}`
        .toLowerCase()
        .indexOf(query)
