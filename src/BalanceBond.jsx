...
const {
  denominations,
  denominationInfo
} = require("oo7-substrate").denominationInfo;

function formatValueNoDenom(n) {
  return `${n.units.toString().replace(/(\d)(?=(\d{3})+$)/g, "$1,")}${
...
class BalanceBond extends InputBond {
  ...
  getUnits() {
    return this.state.ok
      ? denominations[
          this.state.internal ? this.state.internal.denom : defaultDenom()
        ]
      : defaultDenom();
  }
  ...
  validator: (u, s) => {
    let q =
      u === ""
        ? {
            denom:
              s.internal && s.internal.denom !== null
                ? s.internal.denom
                : defaultDenom(),
            units: "0",
            decimals: "",
